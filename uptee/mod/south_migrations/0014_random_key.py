    ...
    def forwards(self, orm):
        for server in orm.Server.objects.all():
            ...
            if os.path.exists(old_path):
                for _file in os.listdir(old_path):
                    move(os.path.join(old_path, _file), os.path.join(new_path, _file))

    def backwards(self, orm):
        for server in orm.Server.objects.all():
            old_path = os.path.join(MEDIA_ROOT, 'mods', server.mod.title, 'servers', server.owner.username, '{0}'.format(server.id), server.random_key)
            new_path = os.path.join(MEDIA_ROOT, 'mods', server.mod.title, 'servers', server.owner.username, '{0}'.format(server.id))
            if os.path.exists(old_path):
                for _file in os.listdir(old_path):
                    move(os.path.join(old_path, _file), os.path.join(new_path, _file))
                rmtree(old_path)
